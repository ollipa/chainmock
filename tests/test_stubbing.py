...
class StubbingTestCase:
    def test_stubbing(self) -> None:
        ...
        assert stub.method("foo") == "stubbed"  # ty: ignore[unresolved-attribute]

    def test_stubbing_arguments_fail(self) -> None:
        stub = mocker().mock("method").called_once_with("foo").return_value("stubbed").self()
        assert stub.method("bar") == "stubbed"  # ty: ignore[unresolved-attribute]
        with assert_raises(
            AssertionError,
            re.compile(
    ...
    def test_stubbing_call_count_fail(self) -> None:
        ...
        assert stub.method() == "stubbed"  # ty: ignore[unresolved-attribute]
        with assert_raises(
            AssertionError,
    ...
    def test_stub_chaining(self) -> None:
        ...
        assert stub.method().another_method() == "stubbed"  # ty: ignore[unresolved-attribute]
    ...
    async def test_stub_async_method(self) -> None:
        ...
        assert await stub.async_instance_method() == "stubbed"  # ty: ignore[unresolved-attribute]
        stub = mocker(spec=SomeClass).mock("async_class_method").return_value("stubbed").self()
        assert await stub.async_class_method() == "stubbed"  # ty: ignore[unresolved-attribute]
        stub = mocker(spec=SomeClass).mock("async_static_method").return_value("stubbed").self()
        assert await stub.async_static_method() == "stubbed"  # ty: ignore[unresolved-attribute]
    ...
    async def test_stub_force_async(self) -> None:
        ...
        assert await stub.instance_method() == "stubbed"  # ty: ignore[unresolved-attribute]

    async def test_stub_force_async_no_spec(self) -> None:
        stub = mocker()
        stub.mock("some_method", force_async=True).return_value("stubbed").called_once()
        assert await stub.some_method() == "stubbed"  # ty: ignore[unresolved-attribute]
    ...
    async def test_stub_force_async_non_existing_attribute(self) -> None:
        ...
        assert await stub.unknown_attr() == "stubbed"  # ty: ignore[unresolved-attribute]
    ...
    def test_stub_kwargs_without_spec(self) -> None:
        ...
        assert stub.some_property == "foo"  # ty: ignore[unresolved-attribute]
        assert stub.instance_method == "bar"  # ty: ignore[unresolved-attribute]
    ...
    def test_stub_property_without_spec(self) -> None:
        ...
        assert stub.some_method("bar") == "baz"  # ty: ignore[unresolved-attribute]
    ...
    def test_stub_property_with_spec(self) -> None:
        ...
        assert stub.instance_method("bar") == "baz"  # ty: ignore[unresolved-attribute]
    ...
    def test_stub_property_with_spec_force_property(self) -> None:
        ...
        assert stub.instance_method == "foo"  # ty: ignore[unresolved-attribute]
    ...
    def test_stub_non_existing_attributes(self) -> None:
        ...
        assert stub.unknown_attr() == "foo"  # ty: ignore[unresolved-attribute]
        stub.mock("unknown_property", create=True, force_property=True).return_value("bar")
        assert stub.unknown_property == "bar"  # ty: ignore[unresolved-attribute]
    ...
    def test_stub_with_list_spec(self) -> None:
        ...
        assert stub.foo() == "foo"  # ty: ignore[unresolved-attribute]
        ...
        assert stub.baz() == "baz"  # ty: ignore[unresolved-attribute]
